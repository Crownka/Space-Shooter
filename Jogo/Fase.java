...
public class Fase extends JPanel implements ActionListener { // classe da fase
    ...
    private int pontuacao; // pontuação do jogo
    ...
    public Fase() { // construtor da fase
        ...
        enemies = new ArrayList<Enemy>(); // cria a lista dos Enemy
    ...
    public void actionPerformed(ActionEvent e) { // atualiza a fase
        ...
        List<Enemy> enemiesToRemove = new ArrayList<>(); // lista de inimigos para remover
        for (Enemy enemy : enemies) {
            if (enemy.isVisivel()) {
                enemy.update();
            } else {
                enemiesToRemove.add(enemy); // adiciona o inimigo na lista de inimigos para remover
                pontuacao += 50; // MUDAR PONTUAÇÃO AQUI
            }
        }

        enemies.removeAll(enemiesToRemove); // remove todos os inimigos da lista de inimigos para remover
    ...
    // tive que mudar o nome da função para checkCollisions, pois a função
    // checkCollision já existe na classe Rectangle
